...
def A_h(Nx: int, Ny: int, h: float, alpha: float):
    diag_0 = np.concatenate(
        [
            np.ones(Nx + 1),
            *[
                np.concatenate([[1], np.ones(Nx - 1) * 4 / (h**2) + np.ones(Nx - 1) * alpha, [1]])
                for _ in range(Ny - 1)
            ],
            np.ones(Nx + 1),
        ]
...
def main():
    ...
    for i in range(nb_iter):
        F1 = RHS(xx1, yy1, right_boundary=u2_mult[i, :, N1 + N2 - N])
        u1_mult[i + 1] = np.reshape(lg.spsolve(A1, F1), (N + 1, N1 + 1))

        F2 = RHS(xx2, yy2, left_boundary=u1_mult[i + 1, :, N - N2])
    ...
    for i in range(nb_iter):
        F1 = RHS(xx1, yy1, right_boundary=u2_add[i, :, N1 + N2 - N])
        u1_add[i + 1] = np.reshape(lg.spsolve(A1, F1), (N + 1, N1 + 1))

        F2 = RHS(xx2, yy2, left_boundary=u1_add[i, :, N - N2])
    ...
    ax1.set_title(f"Iteration {nb_iter//2}")
    ax1.plot_surface(xx1, yy1, u1_mult[nb_iter//2], cmap=colormaps["Blues"])
    ax1.plot_surface(xx2, yy2, u2_mult[nb_iter//2], cmap=colormaps["Greens"])
    ax1.set_aspect("equal")

    ax2.set_title(f"Iteration {nb_iter}")
    ax2.plot_surface(xx1, yy1, u1_mult[-1], cmap=colormaps["Blues"])
    ax2.plot_surface(xx2, yy2, u2_mult[-1], cmap=colormaps["Greens"])
    ...
    fig.tight_layout()
    fig.savefig(f"{image_dir}/with_overlap_graphs_mult")
    ...
    ax1.set_title(f"Iteration {nb_iter//2}")
    ax1.plot_surface(xx1, yy1, u1_add[nb_iter//2], cmap=colormaps["Blues"])
    ax1.plot_surface(xx2, yy2, u2_add[nb_iter//2], cmap=colormaps["Greens"])
    ax1.set_aspect("equal")
    ...
    fig.tight_layout()
    fig.savefig(f"{image_dir}/with_overlap_graphs_add")
    ...
    ax3 = fig.add_subplot()
    ax3.plot(range(nb_iter+1), [err_inf(u1_mult[i], u_exact(xx1, yy1)) for i in range(nb_iter+1)], label="Multiplicative method", marker="+")
    ax3.plot(range(nb_iter+1), [err_inf(u1_add[i], u_exact(xx1, yy1)) for i in range(nb_iter+1)], label="Additive method", marker="+")
    ax3.set_xticks(range(0, nb_iter+1, 5))
    ...
    fig.tight_layout()
    fig.savefig(f"{image_dir}/with_overlap_error")
